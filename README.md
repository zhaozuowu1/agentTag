...
开发环境默认用飞书 Node SDK **长连接**收事件（`im.message.receive_v1`、进群/出群），与开放平台「使用长连接接收事件」对齐，可不配 `FEISHU_ENCRYPT_KEY`。生产请设 `NODE_ENV=production`，仍走 `POST /feishu/events`，并配置 Encrypt Key。可用 `FEISHU_EVENT_MODE=websocket|http` 覆盖默认。
